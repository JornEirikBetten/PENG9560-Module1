...
def lineplot_with_stds(x, y, y_stds, label, color, xlabel, ylabel, figname, xticks = None, alpha = 0.5):
    fig = plt.figure() 
    ax = plt.gca() 
    ...
    ax.fill_between(x, y - 1.96 * y_stds, y + 1.96 * y_stds, color = color, alpha = alpha)
    if xticks != None: 
...
def multiple_lineplots_with_stds(xs, ys, y_stds, labels, colors, xlabel, ylabel, figname, xticks = None, alpha = 0.5):
    fig = plt.figure() 
    ax = plt.gca() 
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    ...
    for i, y in enumerate(ys): 
        plt.plot(xs[i], y, color = colors[i], linestyle = "solid", label = labels[i], alpha = alpha)
